...
class Normalizer(nn.Module):
    ...
    def forward(self, array):
        ...
        self.mean = self.mean.to(array.device)
        self.variance = self.variance.to(array.device)
    ...
    def update(self, array):
        ...
        while array.ndim <= 1:
            array = array.unsqueeze(0).to(device)
        new_mean = torch.mean(array, 0).to(device)
        new_var = torch.var(array, 0).to(device)
        if torch.any(torch.isnan(new_var)):
            new_var = torch.zeros_like(new_var).to(device)
        new_count = (torch.tensor(1.0) * torch.tensor(array.shape[0])) #.to(array.device)
