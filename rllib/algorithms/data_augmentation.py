...
class DataAugmentation(Dyna):
    ...
    def forward(self, observation):
        ...
        if len(self.sim_memory) < batch_size:
            return real_loss
        sim_observation = self.sim_memory.sample_batch(batch_size, device=self.device)[0]
    ...
    def _sample_initial_states(self):
        ...
        obs, *_ = self.memory.sample_batch(self.num_memory_samples, device=self.device)
        ...
        if self.num_initial_state_samples > 0:
            initial_states_ = self.initial_state_dataset.sample_batch(
                self.num_initial_state_samples, device=self.device
            )
