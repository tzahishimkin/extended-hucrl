...
class CEMShooting(MPCSolver):
    ...
    def get_candidate_action_sequence(self):
        ...
        action_distribution = MultivariateNormal(loc=self.mean, covariance_matrix=self.covariance)
        action_distribution._unbroadcasted_scale_tril = action_distribution._unbroadcasted_scale_tril.to(self.mean.device)
    ...
    def update_sequence_generation(self, elite_actions):
        ...
        self.mean = self.alpha * self.mean.to(new_mean.device) + (1 - self.alpha) * new_mean
        self.covariance = self.alpha * self.covariance.to(new_cov.device) + (1 - self.alpha) * new_cov
