...
def step_model(
    ...
    try:
        next_state_tril = dynamical_model.nst_unc_scale
...
def rollout_episode(environment, agent, max_steps, render):
    ...
    time_step = 0
    while not done:
        action = agent.act(torch.tensor(state, dtype=torch.get_default_dtype(), device=agent.device))
        # action = agent.act(torch.tensor(state, device=agent.device))
