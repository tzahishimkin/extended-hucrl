...
def integrate(function, distribution, out_dim=None, num_samples=15):
    ...
    if distribution.has_enumerate_support:
        for action in distribution.enumerate_support():
            ...
            if out_dim is None:
                prob = prob.squeeze(-1)
            ans = ans.to(f_val.device) + prob.detach() * f_val
        ...
        for _ in range(num_samples):
            ...
            ans = ans.to(f_val.device) + f_val / num_samples
    return ans
...
def sample_mean_and_cov(sample, diag=False):
    ...
    if diag:
        ...
        sigma += 1e-6 * torch.eye(sigma.shape[-1]).to(sigma.device)  # Add some jitter.
...
def safe_cholesky(covariance_matrix, jitter=1e-6):
    ...
    try:
        ...
        if jitter > 1:
            ...
            return torch.eye(dim).to(covariance_matrix.device)
        return safe_cholesky(
            covariance_matrix + jitter * torch.eye(dim).to(covariance_matrix.device), jitter=10 * jitter
        )
