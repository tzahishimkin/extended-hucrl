...
def discount_sum(rewards, gamma=1.0, reward_transformer=RewardTransformer()):
    ...
    if rewards.dim() == 0:
        ...
        steps = len(rewards)
        return (
            torch.pow(gamma * torch.ones(steps), torch.arange(steps)).to(rewards.device), * rewards
        ).sum()
    else:
        steps = rewards.shape[1]
        return torch.einsum(
            "i,ki...->k...",
            torch.pow(gamma * torch.ones(steps), torch.arange(steps)).to(rewards.device),
            rewards,
        )
...
def n_step_return(
    ...
    discount = discount.to(rewards[0].device)
