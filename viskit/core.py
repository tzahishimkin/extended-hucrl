...
def to_json(stub_object):
    from misc.instrument import StubObject
    from misc.instrument import StubAttr
...
def load_exps_data(exp_folder_paths, disable_variant=False):
    if PROJECT == 'TD3':
        return load_exps_data_td3(exp_folder_paths, disable_variant)
    elif PROJECT == 'HUCRL':
        return load_exps_data_hcurl(exp_folder_paths, disable_variant)
    else:
        assert 'unknown project'

def load_exps_data_td3(exp_folder_paths,disable_variant=False):
    exps = []
...
def load_exps_data_hcurl(exp_folder_paths,disable_variant=False):
    exps = []
    for exp_folder_path in exp_folder_paths:
        exps += [x[0] for x in os.walk(exp_folder_path)]
    exps_data = []
    for exp in exps:
        try:
            ...
            param_file = "hparams.json" # "params.json"
            progress_file = "statistics.json" #"progress.csv"
            params_json_path = os.path.join(exp_path, param_file)
            variant_json_path = os.path.join(exp_path, "variant.json")
            progress_csv_path = os.path.join(exp_path, progress_file)

            with open(progress_csv_path, 'r') as f:
            ...
            progress = to_dict_list(progress)
